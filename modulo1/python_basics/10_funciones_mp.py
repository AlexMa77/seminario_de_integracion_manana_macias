...
def saludar():
    print("Hola desde el sistema academico")
...
def saludarConNombre(nombre):
    print(f"Hola: {nombre} que tal en clases?")

saludarConNombre("Jhon")
...
def presentar(nombre, edad, curso):
    print(f"{nombre},{edad}, {curso}")

presentar("Alex", 19, "Programacion")  # por posicion
presentar(curso="Matematica", nombre="Juan", edad=40)  # por nombre
...
mostrar_info("materias", "matematica", "lenguaje", "ingles", "programacion")
...
configurar("servidor-academico", 80, 443, 8080, debug=True, timeout=30, ssl=True)
...
notas = [12, 18, 14, 15, 13, 16]
stats = analizar(notas)
